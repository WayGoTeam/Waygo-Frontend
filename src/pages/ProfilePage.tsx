...
const getVehicleConfig = (t: any) => ({
  PETROL: { label: t.profilePage.vehicleTypes.PETROL, icon: Fuel, multiplier: '1x', color: 'text-slate-500' },
  DIESEL: { label: t.profilePage.vehicleTypes.DIESEL, icon: Fuel, multiplier: '1.16x', color: 'text-slate-600' },
        ...
        <div className="p-6 sm:p-8">
          ...
          {step === 'OTP' && (
            <form onSubmit={handleVerifyAndSave} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700">6 rəqəmli kod</label>
                <input
                  type="text"
                  inputMode="numeric"
                  required
                  ...
                  placeholder="------"
                  className="mt-1 block w-full rounded-xl border border-slate-200 px-4 py-2.5 text-center tracking-[0.5em] text-slate-900 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                />
              </div>
              <button
                type="submit"
        ...
        <div className="p-6 sm:p-8">
          ...
          {step === 'CONNECT' && (
            <div className="flex justify-center py-4">
              {loading ? (
                <div className="flex h-[40px] w-full items-center justify-center rounded-full bg-slate-50">
    ...
    <div className="flex h-full flex-col bg-slate-50/50 dark:bg-slate-950">
          ...
          <div className="rounded-3xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl shadow-2xl shadow-slate-200/50 dark:shadow-none ring-1 ring-slate-100 dark:ring-slate-800 p-6 sm:p-8">
            ...
            {!isAdmin && (
              <div className="mt-8 grid gap-8 sm:grid-cols-2">
                ...
                {/* Vehicle info */}
                <div className="space-y-4">
                  <h4 className="font-display text-lg font-bold text-slate-900">{t.profilePage.vehicleInfo}</h4>
                  <div className="group rounded-2xl border border-slate-100 bg-slate-50 p-6 hover:bg-white hover:shadow-md hover:border-slate-200 transition-all relative overflow-hidden">
                    ...
                    <div className="relative flex items-center justify-between mb-6">
                      <div className="flex items-center gap-4">
                        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-sm border border-slate-100 group-hover:scale-105 group-hover:shadow transition-all">
                          <VehicleIcon className={`h-7 w-7 ${config.color}`} />
                        </div>
                        <div>
                          <p className="text-base font-bold text-slate-900">{config.label}</p>
                          <p className="text-xs font-medium text-slate-500">{t.profilePage.engineType}</p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end">
                        ...
                        <span className="mt-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wide">{t.profilePage.ecoPointGain}</span>
                      </div>
                    </div>
                    {user.plateNumber && (
                      <div className="relative mt-4 border-t border-slate-200 pt-5 flex justify-between items-center">
                        <span className="text-xs font-medium text-slate-500">{t.profilePage.plateNumber}:</span>
                        <span className="font-mono text-sm font-bold bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">{user.plateNumber}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="mt-8 block sm:hidden">
              <button
                onClick={() => setIsLogoutModalOpen(true)}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-brand-600 shadow-sm border border-brand-100 hover:bg-brand-50 transition-all"
              >
      ...
      {isLogoutModalOpen && (
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg animate-fade-up rounded-[2rem] bg-white shadow-2xl p-8 sm:p-10 overflow-hidden ring-1 ring-slate-100">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <h2 className="font-display text-3xl font-extrabold text-slate-900 mb-2">{t.profilePage.logoutConfirmTitle}</h2>
              <p className="text-slate-500 text-lg mb-10 max-w-sm mx-auto">{t.profilePage.logoutConfirmDesc}</p>
              <div className="flex w-full flex-col sm:flex-row justify-center gap-4">
                <button onClick={() => setIsLogoutModalOpen(false)} className="flex-1 rounded-2xl px-6 py-4 text-base font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all">
