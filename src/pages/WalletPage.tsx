      ...
      <div className="flex h-full flex-col overflow-y-auto bg-slate-50">
        <div className="mx-auto flex w-full max-w-4xl flex-col items-center justify-center min-h-[60vh] p-6 text-center">
          ...
          <h2 className="mb-3 font-display text-2xl font-bold text-slate-900">
            {t.walletPage.loggedOutTitle}
          </h2>
          <p className="max-w-md text-slate-500">
            {t.walletPage.loggedOutDesc}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-slate-50">
      <div className="mx-auto w-full max-w-4xl p-6">
        ...
        <p className="mt-2 text-slate-500">{t.walletPage.subtitle}</p>
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-600 to-brand-800 p-8 text-white shadow-xl">
            ...
            <div className="relative">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-display text-xl font-medium">{t.walletPage.balanceTitle}</h2>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/20 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-brand-200">{t.walletPage.totalCo2}</p>
        ...
        {vouchers.length > 0 && (
          <div className="mt-12">
            <h3 className="font-display text-xl font-bold text-slate-900 mb-6">{t.walletPage.myVouchers}</h3>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {vouchers.map((v, i) => (
                <div key={i} className="rounded-2xl border border-orange-200 bg-orange-50 p-6 relative overflow-hidden">
                  ...
                  <p className="text-sm font-medium text-orange-600 mb-1">{t.walletPage.fuelCard}</p>
                  <p className="font-mono text-xl font-bold tracking-wider text-slate-900 mb-4">{v.voucherCode}</p>
                  <p className="text-xs text-slate-500">
                    {t.walletPage.issuedAt}: {new Date(v.issuedAt).toLocaleDateString()}
                  </p>
                </div>
              ))}
            </div>
