  ...
  return (
    <div className="scroll-thin h-full overflow-y-auto p-4 sm:p-6">
      ...
      {loading && !data ? (
        ...
        <>
          <div className="mt-5 h-72 overflow-hidden rounded-2xl border border-slate-200 shadow-sm sm:h-96">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <div className="mt-3 space-y-3">
                {data.metro.lines.map((line) => {
                  ...
                  return (
                    <div key={line.name} className="rounded-xl border border-slate-100 p-3">
                      ...
                      <p className="mt-1.5 text-xs text-slate-500">{stations.map((st) => st.name).join(' · ')}</p>
                    </div>
                  )
                })}
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <div className="mt-3 space-y-3">
                {data.buses.map((route) => (
                  <div key={route.name} className="rounded-xl border border-slate-100 p-3">
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-2 text-sm font-medium text-slate-700">
                    ...
                    <p className="mt-1.5 text-xs text-slate-500">{route.stops.join(' · ')}</p>
                  </div>
                ))}
              </div>
            </div>
